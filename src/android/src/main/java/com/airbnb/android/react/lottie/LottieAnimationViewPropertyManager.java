...
/**
 * Class responsible for applying the properties to the LottieView. The way
 * react-native works makes it impossible to predict in which order properties
 * will be set, also some of the properties of the LottieView needs to be set
 * simultaneously.
 * <p>
 * To solve this, instance of this class accumulates all changes to the view and
 * applies them at the end of react transaction, so it could control how changes
 * are applied.
 */
public class LottieAnimationViewPropertyManager {
    ...
    /**
     * Should be set to true if one of the animationName related parameters has
     * changed as a result of last reconciliation. We need to update the animation
     * in this case.
     */
    ...
    /**
     * Updates the view with changed fields. Majority of the properties here are
     * independent so they are has to be reset to null as soon as view is updated
     * with the value.
     * <p>
     * The only exception from this rule is the group of the properties for the
     * animation. For now this is animationName and cacheStrategy. These two
     * properties are should be set simultaneously if the dirty flag is set.
     */
